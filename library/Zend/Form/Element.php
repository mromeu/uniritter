...
class Zend_Form_Element implements Zend_Validate_Interface
{
        ...
        if (empty($decorators)) {
            $getId = create_function('$decorator',
                                     'return $decorator->getElement()->getId()
                                             . "-element";');
            $this->addDecorator('ViewHelper', array('tag' => 'div'))
                 ->addDecorator('Errors', array('tag' => 'div', 'class' => 'text-danger text-left error'))
     ...
     * @throws Zend_Form_Exception for invalid $name values
     */
    public function removeAttrib($name, $value = null)
    {
        $name = (string) $name;
        if ('_' == $name[0]) {
            require_once 'Zend/Form/Exception.php';
